...
print("Select a file")
root = Tk()
root.filename =  filedialog.askopenfilename(initialdir = "/",title = "Select photo to recognize faces",filetypes = (("jpeg files","*.jpg"),("all files","*.*")))
print(root.filename)
root.destroy()
...
for i in range(0,len(pic_enc)):
    ...
    draw.rectangle((left, bottom, left + font.getsize(persname)[0] , bottom +  font.getsize(persname)[1]*1.2), fill='black')
...
if len(unknown_faces_enc) > 0:
    print("There is(are)", len(unknown_faces_enc), "unknown person(s) in the photo. Would you like to enter their information? (Y|N)")
    if input().lower()in ['y','yes']:
        ...
        for i in range(0,len(unknown_faces_location)):
            ...
            if name in people:
                ...
                try:os.mkdir("People/"+name)
                except:pass
                roi.save(os.getcwd()+"/People/"+name+"/"+name+".jpeg","JPEG")
                print("New person added")
            else:print("Person skipped")
